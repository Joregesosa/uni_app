...
<div class="flex w-screen h-screen absolute bg-[#000000a3] left-0 top-0 right-0 bottom-0 z-10">
    ...
    <div class="flex flex-col w-[440px]  shadow-lg rounded-xl border m-auto shadow-cyan-800 p-8 bg-gray-50 relative">
        ...
        <form action="?controller=AdminController&action=list_subjects&modal=admin_edit_subject&row=<?= $row ?>&modal_act=edit_subject&subject=<?= $data[$row]['subject_id'] ?>" method="POST" class="flex flex-col gap-8 mx-auto py-6 h-fit w-full">

            <div>

                <label for="email" class="text-xs">Nombre de la Clase</label>

                <input type="text" name="subject" id="subject" value="<?= $data[$row]['subject'] ?>" placeholder="Clase" required class="outline-none px-3 w-full flex border h-10 rounded-md shadow-md shadow-cyan-950 bg-gray-200 text-gray-600">

            </div>

            <div class="flex flex-col pb-3">

                <label for="teacher" class="text-xs">Maestro Asinado</label>
                ...
                <select name="teacher" id="teacher" class="outline-none px-3 w-full flex border h-10 rounded-md shadow-md shadow-cyan-950 bg-gray-200 text-gray-600">
                    ...
                    <option value="<?= $data[$row]['teacher_id'] ?>">
