...
<div class="flex w-screen h-screen absolute bg-[#000000a3] left-0 top-0 right-0 bottom-0 z-10">
  <div class="flex flex-col w-[440px] shadow-lg rounded-xl border m-auto shadow-cyan-800 p-8 bg-gray-50 relative">
    ...
    <form action="?controller=AdminController&action=list_students&modal=admin_edit_student&modal_act=edit_student&row=<?=$row?>&id=<?= $data[$row]['user_id']?>"  method="POST" class="flex flex-col gap-6 mx-auto py-6 h-fit w-full">

      <div>
        <label for="dni" class="text-xs">DNI</label>

        <input type="text" name="dni" id="dni" placeholder="DNI" required value="<?= $data[$row]['dni'] ?>" class="outline-none px-3 w-full flex border h-10 rounded-md shadow-md shadow-cyan-950 bg-gray-200 text-gray-600" />
      </div>

      <div>
        <label for="email" class="text-xs">Correo Electronico</label>

        <input type="email" name="email" id="email" placeholder="Correo" required value="<?= $data[$row]['email'] ?>" class="outline-none px-3 w-full flex border h-10 rounded-md shadow-md shadow-cyan-950 bg-gray-100 text-gray-600" />
      </div>
      ...
      <div class="flex w-full justify-end">
        <a href="?controller=AdminController&action=list_students" class="shadow-md shadow-cyan-950 bg-cyan-500 rounded-md text-gray-700 text-lg font-medium hover:text-gray-50 mx-auto px-8 py-2">Cancelar</a>
