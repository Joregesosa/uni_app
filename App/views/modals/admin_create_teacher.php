<div class="flex w-screen h-screen absolute bg-[#000000a3] left-0 top-0 right-0 bottom-0 z-10">
  <div class="flex flex-col w-[440px] shadow-lg rounded-xl border m-auto shadow-cyan-800 p-8 bg-gray-50 relative">
    ...
    <form action="?controller=AdminController&action=list_subjects&modal=admin_create_teacher&modal_act=create_teacher" method="POST" class="flex flex-col gap-6 mx-auto py-6 h-fit w-full">
      <div>
        <label for="email" class="text-xs">Correo Electronico</label>

        <input type="email" name="email" id="email" placeholder="Correo" required class="outline-none px-3 w-full flex border h-10 rounded-md shadow-md shadow-cyan-950 bg-gray-200 text-gray-600" />
      </div>
